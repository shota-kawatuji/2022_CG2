...
#pragma region DirectX初期化処理
	...
	for (size_t i = 0; i < backBuffers.size(); i++) {
		...
		// 裏か表かでアドレスがずれる
		rtvHandle.ptr += i * device->
			GetDescriptorHandleIncrementSize(rtvHreapDesc.Type);
		// レンダーターゲットビューの設定
	...
	result = directInput->
		CreateDevice(GUID_SysKeyboard, &keyboard, NULL); // JoystickやSysMouse等の指定も出来る
	assert(SUCCEEDED(result));
...
#pragma region グラフィックスコマンド
		...
		// 描画コマンド
		// commandList->DrawInstanced(_countof(vertices), 1, 0, 0); // 全ての頂点を使って描画(インデックスバッファのみで描画する場合)
		commandList->DrawIndexedInstanced(_countof(indices), 1, 0, 0, 0); // 全ての頂点を使って描画(インデックスバッファと頂点バッファで描画する場合)
		// ４.描画コマンドここまで
#pragma endregion
		// ５.リソースバリアを戻す
